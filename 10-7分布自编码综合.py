# -*- coding: utf-8 -*-
"""
Created on Wed Jul 11 10:58:08 2018
...
savedir = 'log/'
saver = tf.train.Saver(max_to_keep=1)   #保存一个模型
load_epoch = 49
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if epoch % disp_stp == 0:
            ...
            pass
        #保存模型
        saver.save(sess,savedir + 'stacked_auto_encoder.cpkt',global_step=epoch)
        pass
    print("完成训练")
...
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    #读入上层的保存模型
    saver.restore(sess,savedir+ 'stacked_auto_encoder.cpkt-'+str(load_epoch))
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batch):
            ...
            l1_h = sess.run(l1_out,feed_dict={x:batch_xs,y:batch_ys,dropout_keep_prob:1.})
