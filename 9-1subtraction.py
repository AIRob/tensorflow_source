# -*- coding: utf-8 -*-
"""
Created on Wed Jun 27 16:28:44 2018
...
###计算0~256的二进制表示
larget_number = pow(2,binary_dim)
binary = np.unpackbits(
                        np.array([range(larget_number)],dtype=np.uint8).T,
                        axis = 1 )
...
#初始化网络
synapse_0 = (2*np.random.random((input_dim,hidden_dim)) - 1)*0.05  #维度为2*16,2是输出维度，16为隐藏维度
...
for j in range(10000):
    #生成一个数字a
    a_int = np.random.randint(larget_number)
    #生成一个数字b,b的最大值为larget_number/2,作为被减数，让它小一点
    ...
    for position in range(binary_dim):      #循环遍历每一个二进制位
        ...
        layer_1 = sigmoid(np.dot(X,synapse_0) + np.dot(layer_1_values[-1],synapse_h))  #（输入层 + 之前的隐藏层） -> 新的隐藏层，这是体验循环神经网络最核心的地方
        # output layer (new binary representation )
        layer_2 = sigmoid (np.dot(layer_1,synapse_1))       #隐藏层到输出层的转换矩阵 synapse_1 -> 输出层
        
        layer_2_error = y - layer_2             #预测误差
        layer_2_deltas.append((layer_2_error)*sigmoid_output_to_derivative(layer_2))    #把每个时间点的误差导数都记录下来
    ...
    #反向传播，从最后一个时间点 到 第一个时间点   
    for position in range(binary_dim):
        X = np.array([[a[position],b[position]]])   #最后一个的两个输入
        layer_1 = layer_1_values[-position-1]       #当前时间点的隐藏层
        prev_layer_1 = layer_1_values[-position -2] #前一个时间点的隐藏层
        
        layer_2_delta = layer_2_deltas[-position-1] #当前时间点输出层层数
        #通过后一个时间点（因为是反向传播） 的隐藏层误差 和 当前时间点的输出层误差，计算当前时间点的隐藏层误差
