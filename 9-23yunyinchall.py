# -*- coding: utf-8 -*-
"""
Created on Fri Jun 29 17:22:12 2018
...
##自定义
yuyinutils = __import__('9-24yuyinutils')
#from yuyinutils import sparse_tuple_to_texts_ch,ndarray_to_text_ch
#from yuyinutils import get_audio_and_transcriptch,pad_sequences
#from yuyinutils import sparse_tuple_from
...
label_file = '../thchs30/data_thchs30/doc/train.word.txt'

wav_files ,labels = get_wavs_lables(wav_path,label_file)
wav_files = wav_files[:8911]
labels = labels[:8911]
...
def next_batch(labels,start_idx = 0 , batch_size = 1 ,wav_files= wav_files):
    ...
    #print('len txt_labels:',len(txt_labels),type(labels))
    ...
    source ,source_lengths = pad_sequences(source) #如果有多个文件将长度统一，支持按最大截断补0
...
def BiRNN_model(batch_x,seq_length,n_input,n_context,n_character,keep_dropout):
    ...
    with tf.name_scope('lstm'):
        ...
        #连接 正 、 反 结果 [n_steps,batch_size , 2*n_cell_dim]
        outputs = tf.concat(outputs,2)
        #转化形状 [n_steps*batch_size,2*n_cell_dim]
    ...
    with tf.name_scope('fc6'):
        ...
        #将二维 [ n_steps*batch_size,n_character] 转成 三维 time-major [ n_steps,batch_size,n_character]
    layer_6 = tf.reshape(layer_6,[-1,batch_x_shape[0],n_character])
        #pass
    
    #output shape:三维 time-major [ n_steps,batch_size,n_character]
    return layer_6
pass

#语音 MFCC features
input_tensor = tf.placeholder(tf.float32,[None,None,n_input + (2 * n_input * n_context)],name = 'input')
#ctc_loss 计算时需要使用sparse_placeholder 来生成 SparseTensor
...
#调用 ctc_loss
avg_loss = tf.reduce_mean(tf.nn.ctc_loss(targets,logits,seq_length))
...
with tf.name_scope('decode'):
    decoded ,log_prob = tf.nn.ctc_beam_search_decoder(logits,seq_length,merge_repeated = False)
    pass
with tf.name_scope('accuracy'):
    distance = tf.edit_distance( tf.cast(decoded[0],tf.int32),targets)
    #计算 label error rate(accuracy)
...
for epoch in range(epochs):
    ...
    for batch in range(n_batches_per_epoch):
        #每次 取 batch_size 条数据，共循环 n_batches_per_epoch 次
        #取数据
