# -*- coding: utf-8 -*-
"""
Created on Thu Jul  5 15:05:18 2018
...
def build_dataset(words,n_words):
    ...
    for word,nvocab in count:
        ...
        vocab_freqs.append(nvocab)              #加入字典的每个词频  ###############
    data = list()
    unk_count = 0
    for word  in words:
...
vocab_freqs[0] = 90         #将手动添加的第一个词 UNK 的词频调到最大 
sampled = tf.nn.fixed_unigram_candidate_sampler(true_classes=tf.cast(train_labels,tf.int64),num_true=1,num_sampled=num_sampled,unique=True,range_max=words_size,unigrams=vocab_freqs)
...
loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(weights = nce_weights,biases = nce_biases,labels = train_labels,inputs = embed,num_sampled= num_sampled,num_classes=words_size,sampled_values=sampled))
#梯度下降优化器
